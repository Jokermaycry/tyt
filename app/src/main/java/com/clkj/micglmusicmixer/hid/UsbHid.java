package com.clkj.micglmusicmixer.hid;
...
public class UsbHid {
	...
	public static UsbHid mUsbTypeCManager = null;

	
	private static int danOrShuangFlag=2;//
	public UsbHid() {
	 ...
	 * @return
	 */
	public static UsbHid getInstance(Context mContext) {
		synchronized (UsbHid.class) {
			if (mUsbTypeCManager == null) {
				mUsbTypeCManager = new UsbHid();
			}
			mUsbTypeCManager.mContext = mContext;
		}
		return mUsbTypeCManager;
	}
...
public byte[] UsbHidSendCommand(byte[] cmd,int len)
{
	...
	OUT  60 00 02 10
	 byte[] send = { (byte) 0xFF, (byte) 0xAA, (byte) 0x80,(byte) 0x00 };
	**************************************/
	
//	byte[] send_buf = {(byte)0,(byte)0,(byte)0,(byte)0,(byte)0,(byte)0};
//
//
//	System.arraycopy(cmd, 0, send_buf, 1, len);
//	for(int i=0;i<send_buf.length;i++)
//	{
//		Log.e("wwmlength","0x"+Integer.toHexString(send_buf[i] & 0xFF));
//	}
//	Log.e("wwmlength",String.valueOf(send_buf.length));
//	Log.e("wwmlen",String.valueOf(len));

	int res = hid_set_output_report(myDeviceConnection, cmd, len);
...
//   	for (int i = 0; i < res; i++)
//		Log.d("wangweiming","0x"+Integer.toHexString(byteReturn[i] & 0xFF));
//      	}
//
	return byteReturn;
...
private boolean findInterface() {
		...
		if (myUsbDevice != null) {
			...
			int interfaceCount= myUsbDevice.getInterfaceCount();			
			if(mDebug) Log.d(TAG, "interfaceCounts : " + interfaceCount);
			
			if(interfaceCount>=6){//�����6�������ϣ���˫��
				danOrShuangFlag=3;
			}else{
				danOrShuangFlag=2;
			}
			for (int i = 0; i < myUsbDevice.getInterfaceCount(); i++) {
				...
				if(mDebug) Log.d(TAG,"i="+i+",Intf Class="+intf.getInterfaceClass()+",Sub class="+intf.getInterfaceSubclass()	
					 +",Intf Protocol="+intf.getInterfaceProtocol()+",endpointcount="+intf.getEndpointCount());
 ...
 private boolean ConnectDevice() {
	...
	if (myInterface != null) {
		...
		if (myUsbManager.hasPermission(myUsbDevice)) {
			...
			Log.d(TAG,
					"You have not Permission to open the USB HID Device...");
			myUsbManager.requestPermission(myUsbDevice, PendingIntent.getBroadcast(mContext, 0, new Intent("com.android.example.USB_PERMISSION"), 0));
...
private int hid_set_output_report(UsbDeviceConnection conn, byte[] data, int len)
{
	...
	int r3=(2 << 8) | report_number;//512
//	int r3=0x0201;
	res = conn.controlTransfer(requestType,0x09,r3,danOrShuangFlag,data, len,1000);//2�ǵ��3��˫��
...
//	int r3=0x0101;
	res =conn.controlTransfer(requestType,0x01,r3,danOrShuangFlag,data, len,1000);
	if(mDebug) Log.d(TAG,"res="+res);
	if (res < 0){
